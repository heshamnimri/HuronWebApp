...
describe("Signing Certificate with Certificate Authority", function () {
    ...
    it("T3 - should create various Certificates signed by the CA authority", (done) => {
        ...
        async.series([
            ...
            (callback) => {
                sign(certificateRequest, lastYear, 200, (err) => callback(err));
            },
            (callback) => {
                sign(certificateRequest, lastYear, 10 * 365, (err) => callback(err));
            },
            (callback) => {
                sign(certificateRequest, nextYear, 365, (err) => callback(err));
            }
        ], done);
    });
